...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const requiresExplicitSubmit = [
    'essay', 'explain', 'answer', 'fill_in', 'drawing', 'matching', 'classify'
  ...
  return (
    <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200/90 dir-rtl my-4 space-y-6 transition-all">
      ...
      {question.type === 'matching' && question.matchingPairs && (() => {
        ...
        const selectedCount = Object.values(matchingState).filter((v) => v !== '').length;
        ...
        return (
          <div className="space-y-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
              ...
              <span className={`px-2.5 py-1 text-[10px] font-bold rounded-lg ${selectedCount === totalPairs ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                تم اختيار {selectedCount} من {totalPairs}
              </span>
            </div>
                ...
                <div key={pair.id} className="flex flex-col sm:flex-row items-center gap-3 bg-white p-3 rounded-xl border border-slate-200">
                  ...
                  >
                    ...
                    {question.matchingPairs?.map((p) => {
                      const isSelectedElsewhere = Object.entries(matchingState).some(([k, v]) => k !== pair.left && v === p.right);
                      return (
                        <option key={p.id} value={p.right} disabled={isSelectedElsewhere} className={isSelectedElsewhere ? 'text-slate-300 bg-slate-50' : ''}>
                          {p.right} {isSelectedElsewhere ? '(تم اختياره)' : ''}
                        </option>
                      );
                    })}
                  </select>
                ...
                <button
                  ...
                  onClick={() => {
                    if (selectedCount < totalPairs) {
                      alert('عذراً، يرجى إكمال جميع التوصيلات قبل الاعتماد.');
                      return;
                    }
                    if (onConfirmAnswer) {
                      onConfirmAnswer();
      ...
      {question.type === 'classify' && question.classification && (() => {
        const allItems = question.classification.flatMap((c) => c.items);
        const totalItems = allItems.length;
        const assignedItems = allItems.filter(item => 
          question.classification?.some(cat => (classifyState[cat.category] || []).includes(item))
        ).length;
        ...
        return (
          <div className="space-y-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
              ...
              <span className={`px-2.5 py-1 text-[10px] font-bold rounded-lg ${assignedItems === totalItems ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                تم تصنيف {assignedItems} من {totalItems}
              </span>
            </div>
            <div className="space-y-3">
                ...
                <button
                  ...
                  onClick={() => {
                    if (assignedItems < totalItems) {
                      alert('عذراً، يرجى تصنيف جميع العناصر قبل الاعتماد.');
                      return;
                    }
                    if (onConfirmAnswer) {
                      onConfirmAnswer();
      ...
      {['essay', 'explain', 'answer'].includes(question.type) && (
        <div className="space-y-3">
